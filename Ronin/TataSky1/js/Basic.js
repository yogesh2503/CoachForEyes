...
class Runner {
    constructor(texture, shadowTex) {
        this.first = TexSingle(texture, 2, 2, 4, 100);
        this.first.body.scale.set(.01, .02, .02);
    ...
    setshadwow() {
        this.shadow.position.set(this.first.body.position.x, this.first.body.position.y - .2, this.first.body.position.z);
        this.shadow.visible = true;
    }
    update() {
        this.first.body.position.set(this.x, this.y, this.z);
        this.shadow.position.set(this.x, this.sy + .01, this.z);
        if (this.time < MAXLNG || this.gComp == 0) {
            this.first.annie.update(mGame.delta * 1000);
            this.time += this.spd;
            if (this.vx > 0) {
                this.x += this.vx;
                console.log(this.x, this.mx, this.vx);
                if (this.x > this.mx) {
                    this.x = this.mx;
                    this.vx = 0;
                }
            ...
            if (this.spd > 0) {
                ...
                this.gComp++;
                this.shadow.visible = this.first.body.visible = false;
                // DrawTexture(mGame.mTex_disBack2, 0, 550, 3000, 300);
                // DrawTexture(mGame.mTex_disBack, 0, 530, 3000, 180);
                // if (this.gComp < 20) {
                //     DrawTexture(mGame.mTex_dish2, 0, 250, 256 * this.gComp * .05, 512 * this.gComp * .05);
                // } else {
                //     DrawTexture(mGame.mTex_dish2, 0, 250, 256, 512);
                // }
    ...
    tap(dir) {
        // if (this.vy == 0 || this.vx == 0) 
        {
            ...
            if (dir == UP && this.onBase == 0) {
                ...
                // mGame.audiojump.play();
                // mGame.audiojump.loop = false;
            }
        }
    }
}