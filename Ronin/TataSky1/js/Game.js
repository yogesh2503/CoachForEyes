...
class Game {
    ...
    init() {
        ...
        function onError() { }
        var shadowTex = new THREE.TextureLoader().load('assets/shadow.png');
        var backText = new THREE.TextureLoader().load('assets/back.jpg');
        var dishtxt = new THREE.TextureLoader().load('assets/dish2.png');
        var target = new THREE.TextureLoader().load('assets/yellowmark.png');
        var runTexture = new THREE.TextureLoader().load('assets/run.png');
        var rlbf = new THREE.TextureLoader().load('assets/tile/right.jpg');
        var roof = new THREE.TextureLoader().load('assets/tile/top.jpg');
        rooftop = new THREE.TextureLoader().load('assets/tile/top2.jpg');
        rlbf.wrapS = THREE.RepeatWrapping; rlbf.wrapT = THREE.RepeatWrapping; rlbf.repeat.set(2, 30);
        var sideBuildTex = [];
        for (let i = 0; i < 5; i++) {
            sideBuildTex.push(new THREE.TextureLoader().load('assets/tile/br' + i + '.jpg'));
            sideBuildTex[i].wrapS = THREE.RepeatWrapping; sideBuildTex[i].wrapT = THREE.RepeatWrapping; sideBuildTex[i].repeat.set(1, 10);
        }

        var geometry = new THREE.PlaneBufferGeometry(512, 200);
        ...
        var up = new THREE.Mesh(new THREE.PlaneBufferGeometry(512, 64), new THREE.MeshBasicMaterial({ color: 0xffffff }));
        this.mBackPLan.add(up);
        up.position.set(0, 130, .1);
        ...
        // console.log(createColor(1));
        // this.skyTarget.material.color = createColor();
        ...
        this.myAudiobg = document.getElementById("myAudiobgsoundtrack");
        this.myAudiobg.loop = true;
        this.myAudiojump = document.getElementById("myAudiojump");
        this.myAudiocheer = document.getElementById("myAudiocheer");


        this.runner = new Runner(runTexture, shadowTex);
        this.scene.add(this.runner.first.body);
        this.scene.add(this.runner.shadow);
        this.runner.first.body.position.set(0, 0, -20);
        this.runner.first.visible = true;
        ...
        AssetLoader.add.image('assets/dish.png');
        AssetLoader.add.image('assets/dish2.png');
        AssetLoader.add.image('assets/power.jpg');
        AssetLoader.add.image('assets/name.png');
        AssetLoader.add.image('assets/play.png');
        AssetLoader.add.image('assets/booknow.png');

        AssetLoader.add.image('assets/icon1.png');
        AssetLoader.add.image('assets/icon2.png');
        AssetLoader.add.image('assets/icon3.png');

        AssetLoader.add.image('assets/well-done.png');
        AssetLoader.add.image('assets/endscreen.png');
        AssetLoader.add.image('assets/white-image.png');
        AssetLoader.add.image('assets/online-payment.png');

        AssetLoader.add.image('assets/0.png');
        AssetLoader.add.image('assets/1.png');
        AssetLoader.add.image('assets/2.png');
        AssetLoader.add.image('assets/3.png');
        AssetLoader.add.image('assets/rs.png');
        AssetLoader.add.image('assets/power1.png');
        AssetLoader.add.image('assets/trasprent.png');
        ...
        AssetLoader.load(function () {
            ...
            game.mTex_trasprent = loadUI(game.gameUI, 'assets/trasprent.png', 0, 0, 0);
            game.mTex_title = loadUI(game.gameUI, 'assets/title.png', 0, 0, 0);
            game.mTex_power = loadUI(game.gameUI, 'assets/power.jpg', 0, 0, 0);
            game.mTex_power1 = loadUI(game.gameUI, 'assets/power1.png', 0, 0, 0);
            game.mTex_name = loadUI(game.gameUI, 'assets/name.png', 0, 0, 0);
            game.mTex_play = loadUI(game.gameUI, 'assets/play.png', 0, 0, 1);
            game.mTex_book = loadUI(game.gameUI, 'assets/booknow.png', 0, 0, 2);
            game.mTex_icons = [];
            game.mTex_icons.push(loadUI(game.gameUI, 'assets/icon1.png', 0, 0, 0));
            game.mTex_icons.push(loadUI(game.gameUI, 'assets/icon2.png', 0, 0, 0));
            game.mTex_icons.push(loadUI(game.gameUI, 'assets/icon3.png', 0, 0, 0));
            game.mTex_GO = [];
            game.mTex_GO.push(loadUI(game.gameUI, 'assets/3.png', 0, 0, 0));
            game.mTex_GO.push(loadUI(game.gameUI, 'assets/2.png', 0, 0, 0));
            game.mTex_GO.push(loadUI(game.gameUI, 'assets/1.png', 0, 0, 0));
            game.mTex_GO.push(loadUI(game.gameUI, 'assets/0.png', 0, 0, 0));

            game.mTex_welldone = loadUI(game.gameUI, 'assets/well-done.png', 0, 0, 0);
            game.mTex_endscreen = loadUI(game.gameUI, 'assets/endscreen.png', 0, 0, 0);
            game.mTex_whiteimage = loadUI(game.gameUI, 'assets/white-image.png', 0, 0, 0);
            game.mTex_onlinepay = loadUI(game.gameUI, 'assets/online-payment.png', 0, 0, 0);

            game.mTex_disBack2 = loadUIRect(game.gameUI, 0, 0, 256, 256, '#4480bc');
            game.mTex_disBack = loadUIRect(game.gameUI, 0, 0, 256, 256, '#588fcf');

            game.mTex_dish = loadUI(game.gameUI, 'assets/dish.png', 0, 0, 0);
            game.mTex_rs = loadUI(game.gameUI, 'assets/rs.png', 0, 0, 0);
            game.mTex_dish2 = loadUI(game.gameUI, 'assets/dish2.png', 0, 0, 0);
    ...
    animate() {
        ...
        switch (GameScreen) {
            ...
            case GAMEOVER:
                ...
                break;
            case GAMELOAD:
                DrawLbl(mGame.mTex_fonts[0], "Presents", 0, -1460, CLR_BLUE, 38);
                if (this.counter > 5)
    ...
    DrawMenu() {
        ...
        DrawTexture(this.mTex_name, 0, -320, 415 * .9, 230 * .9);
        this.camera.rotation.set(-.07, 0, 0);//30
        this.camera.position.set(0, 1.0, -19);//30



        DrawLbl(mGame.mTex_fonts[0], "Presents", 0, -460, CLR_BLUE, 38);
        DrawLbl(mGame.mTex_fonts[1], "Find the roof to install Tata Sky", 0, -170, CLR_BLUE, 38);
        DrawLbl(mGame.mTex_fonts[2], "and Win exciting offers!", 0, -130, CLR_BLUE, 38);
        DrawTexture(this.mTex_play, 0, 500, 442, 117);
        this.runner.first.body.position.set(0, .4, -21);
        this.mBackPLan.position.set(0, -23, -300);
    ...
    DrawOver() {
        ...
        DrawLbl(mGame.mTex_fonts[2], "Get     400 OFF ON", 0, -220, CLR_BLUE, 38);

        DrawTexture(this.mTex_rs, -65, -232, 32 * .56, 64 * .56);
        DrawTexture(this.mTex_onlinepay, 0, -180, 366 * 1.1, 33 * 1.1);
        ...
        this.skyTarget.visible = this.mTex_dish.visible = this.mTex_power.visible = false;
    }
    DrawGameplay() {
        ...
        for (let i = 0; i < this.mBuilding.length && this.runner.over == 0; i++) {
            ...
            if (CircRectsOverlap(this.mBuilding[i].position.x, this.mBuilding[i].position.z, this.mBuilding[i].scale.x * .5, this.mBuilding[i].scale.z * .5, this.runner.x, this.runner.z, .01)) {
                ...
                if (this.runner.time >= MAXLNG && this.runner.gComp == 0) {
                    this.runner.gComp = 1;
                }
                syi = i;
            }
        ...
        for (let i = 0; i < this.mBuilding.length; i++) {
            if (this.mBuilding[i].position.z > -0) {
                ...
                if (this.mBuilding[i].position.z < -MAXLNG + this.runner.time) {
                    ...
                    console.log("this.mBuilding[i].position.x = " + this.mBuilding[i].position.x);
                }
                console.log("this.mBuilding[i].position.x ~~~~ " + this.mBuilding[i].position.x);
        ...
        for (let i = 0; i < this.mRSide.length; i++) {
            ...
            if (i % 2 == 0)
                this.tatasky[i].position.set(-7 + this.mLSide[i].scale.x * .45, this.mLSide[i].position.y + this.mLSide[i].scale.y * .5 + .3, this.mLSide[i].scale.z * .45 + this.mLSide[i].position.z);
            else
                this.tatasky[i].position.set(7 - this.mLSide[i].scale.x * .45, this.mRSide[i].position.y + this.mRSide[i].scale.y * .5 + .3, this.mLSide[i].scale.z * .45 + this.mLSide[i].position.z);
        }
        ...
        DrawTexture(this.mTex_power1, -184, - 500, 16, 36);
        this.mTargetPlan.position.set(this.mBuilding[rp].position.x, 1.2, -MAXLNG - 20 + this.runner.time);
        ...
        if (this.runner.gComp > 100) {
            setTimeout(() => { confetti.reset(); }, 50);
            this.setScreen(GAMEOVER);
        }
        // console.log(this.mTargetPlan.position);
    ...
    setScreen(scr) {
        ...
        this.mBackPLan.position.set(0, -30, -300);
        switch (GameScreen) {
            case GAMEMENU:
