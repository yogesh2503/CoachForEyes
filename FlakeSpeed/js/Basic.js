...
const FONTSCORE = "#a3c8cf";
const BUTTONFONT = "#f0f0f0";
const FONTBLUE = "#2b3e84"; //const FONTGREEN = "#225b3d";
...
function loadUIS(assetpath, x, y) {
    ...
    sprite.x = x;
    sprite.y = y;
    sprite.width = sprite.width * 0.08;
    sprite.height = sprite.height * 0.08;
    sprite.pivot.x = 0.5;
    sprite.pivot.y = 0.5;
...
function collision(fx, fy, fr, sx, sy, sr) {
    distance = Math.sqrt(((fx - sx) * (fx - sx)) + ((fy - sy) * (fy - sy)));
    if (distance < fr + sr) {
        return true;
    }
    return false;
...
function calcluate(firstBall, secondBall) {
    var nvx1 = (firstBall.sx * (firstBall.mass - secondBall.mass) + (2 * secondBall.mass * secondBall.sx)) / (firstBall.mass + secondBall.mass);
    var nvy1 = (firstBall.sy * (firstBall.mass - secondBall.mass) + (2 * secondBall.mass * secondBall.sy)) / (firstBall.mass + secondBall.mass);
    var nvx2 = (secondBall.sx * (secondBall.mass - firstBall.mass) + (2 * firstBall.mass * firstBall.sx)) / (firstBall.mass + secondBall.mass);
    var nvy2 = (secondBall.sy * (secondBall.mass - firstBall.mass) + (2 * firstBall.mass * firstBall.sy)) / (firstBall.mass + secondBall.mass);

    return [nvx1, nvy1, nvx2, nvy2];
}