...
function init() {
    ...
    AssetLoader.load(function () {
        ...
        mTex_hand.vx = -1;
        mTex_Continue = loadUI('assets/continue.png', 0, 200, 11);
...
function Handle_Game(type) {
    if (mPlayer.anim == 2) {
        if (type == 0) {
            ...
            selObj(no);
            // 
            console.log("[type = " + type + "][mPop.tubeNo = " + mPop.tubeNo + "][mPush.tubeNo = " + mPush.tubeNo + "]");
        }
        if (type == 1) {
            var col = [];
            mPlayer.tubes.forEach(element => { col.push(element.circle); });
    ...
    if (mPush.tubeNo != mPop.tubeNo) {
        ...
        if (mPush.wNo < mTube[mPush.tubeNo].water.length - 1 && mPop.wNo >= 0) {
            if (mPop.noSameClr + mPush.noSameClr <= mTube[mPush.tubeNo].water.length && (mPop.color == mPush.color || mPush.color == -1)) {
                ...
                console.log("{mPop.tubeNo = " + mPop.tubeNo + "}{mPush.tubeNo = " + mPush.tubeNo + "}");
                isMove = true;
            }
...
function Handle_Menu(clickval) {
    console.log(mPlayer.lvl + " clickval = " + clickval)
    if (mPlayer.lvl < 3) {
        mPlayer.lvl++;
        gamereset();
    } else {
