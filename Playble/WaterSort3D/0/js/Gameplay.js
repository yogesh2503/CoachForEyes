...
function init() {
    ...
    for (var i = 0; i < 5; i++) {
      if(i ==0)
        ...
        mPlan_Home.push(mPlan_Home[0].clone());
      scene.add(mPlan_Home[i]);
      mPlan_Home[i].position.set(-400+200*i, -146, -192);
      mPlan_Home[i].rotation.set(-.4, 0, 0);
    }
    for (let i = 0; i < 5; i++) {
        mTube.push(new Tube());
...
function Draw() {
    ...
    if(window.innerWidth>window.innerHeight){
      camera.position.set(0, -60+40, -90+250);
      scx = 0.5;
    }else {
        camera.position.set(0, 40, +250);
      }
    if (mPlayer.gOver == false) {
        ...
        if (mTex_Continue.sx > 1.1+scx) {
            mTex_Continue.vx = .995;
        }
        if (mTex_Continue.sx < 0.99+scx) {
...
function installAnimation(){
  ...
  if(mTex_Download.s>1+scx)
    mTex_Download.sx = .993;
  if(mTex_Download.s<.9+scx)
    ...
    if (mPlayer.anim == 2) {
        if (type == 0) {
            ...
            for (let i = 0; i < mPlayer.tubes.length && no == -1; i++) {
              console.log(mPlayer.tubes[i].group.position.x,(mouse.x * (111+scaleX) * ration),mPlayer.tubes[i].group.position.y,(mouse.y * (900+scaleY)));
                if (CircRectsOverlap(mPlayer.tubes[i].group.position.x, mPlayer.tubes[i].group.position.y, 10, 200, (mouse.x * (111+scaleX) * ration), (mouse.y * (900+scaleY)), 5)) {
                    no = i;
                }
            }
            selObj(no);
        ...
        if (type == 1) {
            ...
            for (let i = 0; i < mPlayer.tubes.length; i++) {
                if (CircRectsOverlap(mPlayer.tubes[i].group.position.x, mPlayer.tubes[i].group.position.y, 10, 200, (mouse.x * (111+scaleX) * ration), (mouse.y * 900), 5)) {
                    newone = i;
                }
            }
        }
