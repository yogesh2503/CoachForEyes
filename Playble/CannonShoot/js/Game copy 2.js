class Game {
	...
	initPhysics() {
		...
		this.shapes.cylinder = new CANNON.Cylinder(3.5, 3.5, this.hight, 20);
		...
		for (let i = 0; i < 28; i++) {
			...
			this.cyl.push(this.addBody(2, x, 4 + (this.hight * .5) + (Math.floor(i / 7) * this.hight), z));
	...
	animate() {
		...
		for (let i = 0; i < this.cyl.length && this.isTuch == false; i++) {
			...
			var x = Math.cos(red) * 9;
			var z = Math.sin(red) * 9;
			this.cyl[i].position.set(x, 4 + (this.hight * .5) + (Math.floor(i / 7) * this.hight), z);
			this.cyl[i].quaternion.setFromAxisAngle(new CANNON.Vec3(1, 0, 0), Math.PI / 2);
		}


		// console.log("this.tuch = " + this.isTuch);
