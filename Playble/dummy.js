...
function parseMessage(e) {
    ...
    if (-1 !== n) { var t = i.slice(n + 2); return getMessageParams(t) }
    return {}
}
...
function init() { window.dapi.isDemoDapi && (window.parent.postMessage(DOLLAR_PREFIX + SEND_MSG_PREFIX + JSON.stringify({ state: "getDapiUrl" }), "*"), window.addEventListener("message", getDapi, !1)) }
var DOLLAR_PREFIX = "$$",
    RECEIVE_MSG_PREFIX = "DAPI_SERVICE:",
    SEND_MSG_PREFIX = "DAPI_AD:",
    GET_DAPI_URL_MSG_NAME = "connection.getDapiUrl",
    dapiEventsPool = {},
    dapi = window.dapi || { isReady: function() { return !1 }, addEventListener: function(e, i) { dapiEventsPool[e] = i }, removeEventListener: function(e) { delete dapiEventsPool[e] }, isDemoDapi: !0 };
init();
...
window.onload = function() {
    console.log("~~~~here you can put other code that not related to dapi logic");
    (dapi.isReady()) ? onReadyCallback(): dapi.addEventListener("ready", onReadyCallback);

};

function onReadyCallback() {
    console.log("~~~~no need to listen to this event anymore");
...
function adVisibleCallback(event) {
    ...
    if (event.isViewable) {
        ...
        console.log("~~~~START or RESUME the ad");
    } else {
        console.log("~~~~PAUSE the ad and MUTE sounds");
    }
}
...
function userClickedDownloadButton(event) {
    console.log("~~~~userClickedDownloadButton");
    dapi.openStoreUrl("https://play.google.com/store/apps/details?id=com.oneday.game24.trainsimulatorencr");
}

function audioVolumeChangeCallback(volume) {
    let isAudioEnabled = !!volume;
    if (isAudioEnabled) {
        console.log("~~~~START or turn on the sound");
    } else {
        console.log("~~~~PAUSE the turn off the sound");
    }
}
