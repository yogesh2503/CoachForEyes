...
var mPlan_Road = [],
    ...
    mPlan_Player = [],
    mPlan_Police = [],
    mPlan_Home, mPlan_Splash, mPlan_Logo, mPlan_ads;
var mTex_hand, mTex_Swipe, mTex_Play, mTex_logo, mTex_Menu, mTex_leader;
var mTex_soundon, mTex_soundoff, mTex_name, mTex_score, mTex_nexticn;
var mTex_gamepaus, mTex_gamewin, mTex_scorebox, mTex_popup, mTex_homeicn, mTex_playicn, mTex_retryicn;
var mTex_platinum, mTex_silvermedal, mTex_bronze, mTex_goldmedal, mTex_Rate, mTex_share;
var mTex_Heart = [],
    mTex_EHeart = [],
    anim = [];
...
function initstart() {
    ...
    tex_Ads = textureLoader.load(BASEURL + 'assets/ads_sqr.png');
    tex_logo = textureLoader.load(BASEURL + 'assets/logo.png');
    ...
    var material = new THREE.MeshBasicMaterial({ map: tex_Ads, transparent: true });
    mPlan_ads = new THREE.Mesh(geometry, material)
    scene.add(mPlan_ads);
    mPlan_ads.visible = false;
    mPlan_ads.position.set(-11, -5, 0);
    ...
    AssetLoader.add.image(BASEURL + 'assets/splashtitle.png');
    ...
    AssetLoader.load(function() {
        ...
        mTex_name = loadUI(BASEURL + 'assets/splashtitle.png', 0, 0, 0.7);
...
function Draw() {
    ...
    switch (GameScreen) {
        case GAMELOGO:
            ...
            break;
        case GAMEMENU:
            DrawTransScalL(mTex_Play, 200, 140, 160, 80, mSel == 1 ? 1.1 : 1, mSel == 1 ? 0.5 : 1);
            DrawTransScalL(mTex_soundon, 100, 220, 160, 80, mSel == 2 ? 1.1 : 1, mSel == 2 ? 0.5 : 1);
            DrawTransScalL(mTex_soundoff, 100, 220, 160, 80, mSel == 2 ? 1.1 : 1, mSel == 2 ? 0.5 : 1);
            DrawTransScalL(mTex_Rate, 180, 320, 100, 100, mSel == 3 ? 1.1 : 1, mSel == 3 ? 0.5 : 1);
            DrawTransScalL(mTex_leader, 60, 320, 128, 128, (mSel == 4 ? 1.1 : 1) * ys, mSel == 4 ? 0.5 : 1);
...
function handle_WinOver(e, type) {
    ...
    if (type == 2) {
        ...
        if (raycaster.intersectObject(mPlan_ads).length > 0) {
            DawnloadUs();
        }
...
function setScreen(scr) {
    ...
    mPlan_ads.visible = mTex_scorebox.visible = false;
    mTex_popup.visible = mTex_homeicn.visible = mTex_playicn.visible = mTex_retryicn.visible = false;
    mTex_nexticn.visible = mTex_leader.visible = false;
    mTex_soundon.visible = mTex_soundoff.visible = mTex_name.visible = false;
    meshLoading.visible = mTex_Menu.visible = mPlan_Splash.visible = mTex_name.visible = false;
    mPlan_Home.visible = mTex_hand.visible = mTex_Swipe.visible = mTex_Play.visible = false;
    ...
    switch (GameScreen) {
        ...
        case GAMESTART:
            ...
            DrawLblA(mTex_fonts[0], "LEVEL : " + mPly.lvl, 20, -314, BUTTONFONT, 20, ThreeUI.anchors.center, ThreeUI.anchors.center, "center");
            // DrawTextureA(mTex_Rolls, 53, 24, ThreeUI.anchors.left, ThreeUI.anchors.bottom);
            DrawLblA(mTex_fonts[4], "0", 45, 12, COLOR1, 24, ThreeUI.anchors.left, ThreeUI.anchors.bottom, "left");
        ...
        case GAMEPLAY:
            ...
            mPly.set();
            DrawLblA(mTex_fonts[0], "LEVEL : " + mPly.lvl, 20, -318, BUTTONFONT, 20, ThreeUI.anchors.center, ThreeUI.anchors.center, "center");
            // DrawTextureA(mTex_Rolls, 53, 24, ThreeUI.anchors.left, ThreeUI.anchors.bottom);
            DrawLblA(mTex_fonts[4], "0", 45, 12, COLOR1, 24, ThreeUI.anchors.left, ThreeUI.anchors.bottom, "left");
        ...
        case GAMEWIN:
            ...
            if (GameScreen == GAMEOVER)
                DrawLbl(mTex_fonts[0], "GAMEOVER", 0, -210, COLOR1, 24);
            if (GameScreen == GAMEPAUSE)
                DrawLbl(mTex_fonts[0], "GAMEPAUSE", 0, -210, COLOR1, 24);
            if (GameScreen == GAMEWIN)
                DrawLbl(mTex_fonts[0], "GAMEWIN", 0, -210, COLOR1, 24);



            DrawTexture(mTex_scorebox, 0, -50);
            DrawLbl(mTex_fonts[1], "SCORE", 0, -120, BUTTONFONT, 30);
            DrawLbl(mTex_fonts[2], "USE A MASK", 0, 50, BUTTONCOL2, 20);
            DrawLbl(mTex_fonts[3], "" + mPly.roll, 0, -38, COLOR0, 30);
        ...
        case GAMELEADER:
            var str1 = "",
                str2 = "";
            mPlan_Road.forEach(element => { element.visible = true; });
            DrawLbl(mTex_fonts[0], "LEADERBOARD", 0, -210, COLOR1, 24);
            ...
            mTex_popup.visible = true;
            DrawLblA(mTex_fonts[2], str1.toUpperCase(), 60., -140, BUTTONFONT, 14, ThreeUI.anchors.center, ThreeUI.anchors.center, "left");
            DrawLblA(mTex_fonts[3], str2.toUpperCase(), 60, -140, BUTTONFONT, 14, ThreeUI.anchors.center, ThreeUI.anchors.center, "right");
            break;
...
function setStore() {
    ...
    try {
        if (isAndroid == true) {
            ...
            // localStorage.setItem("lvl" + sid, 1);
