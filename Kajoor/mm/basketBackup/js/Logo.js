...
function setSCreen(scr) {
    GameScreen = scr;
    mTex_logo.visible = mTex_over.visible = mTex_continue.visible = mTex_exit.visible = mTex_submit.visible = input_Box.visible = false;
...
function Handle_Over(pointer, type) {
    ...
    if (CircRectsOverlap(mTex_continue.x, mTex_continue.y, mTex_continue.width * .5, mTex_continue.height * .5, pointer.x, pointer.y, 1) && isDown) {
        mTex_continue.alpha = .1;
    }
    if (CircRectsOverlap(mTex_submit.x, mTex_submit.y, mTex_submit.width * .5, mTex_submit.height * .5, pointer.x, pointer.y, 1) && isDown) {
        mTex_submit.alpha = .1;
    }
    if (CircRectsOverlap(mTex_exit.x, mTex_exit.y, mTex_exit.width * .5, mTex_exit.height * .5, pointer.x, pointer.y, 1) && isDown) {
        mTex_exit.alpha = .1;
    }
    if (type == 2) {
