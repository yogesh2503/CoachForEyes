...
function init() {
    ...
    var material = new THREE.MeshBasicMaterial({ map: txtPlayer[pNo] });
    ...
    var material = new THREE.MeshBasicMaterial({ map: mTex_BG[random(0, mTex_BG.length)] });
    ...
    AssetLoader.add.image('assets/logo.jpg');
    ...
    AssetLoader.progressListener = function(progress) {
        DrawLblA(mTex_load, "Loading : " + Math.floor(50 + progress * 50) + "%", 0, 100, "#0F0F0F", 55, ThreeUI.anchors.center, ThreeUI.anchors.center, "center");
    };
    AssetLoader.load(function() {
        mTex_logo = loadUIRect('#f7f7f7');
        httlogo = loadUI('assets/logo.jpg', 0, 0, 0);
...
function Draw() {
    ...
    switch (GameScreen) {
        ...
        case GAMEOVER:
            break;
        case GAMELODING:
            meshLoading.rotation.set(Counter * .03, Counter * .06, Counter * .09);
            meshLoading.position.set(0, -15, 50);
            break;
    }
    Counter++;
    if (isResize > 0) {
...
function setScreen(scr) {
    ...
    for (var i = 0; i < mp_BG.length; i++) {
        mp_BG[i].visible = GameScreen != GAMELOGO;
    }
    for (var i = 0; i < planBase.length; i++) {
        planBase[i].visible = false;
        red[i].visible = false;
    ...
    switch (GameScreen) {
        ...
        case GAMEOVER:
            ...
            DrawLblA(mTex_fonts[3], "Diamond score : " + mDimondscore, 0, -40, "#FFD700", 35, ThreeUI.anchors.center, ThreeUI.anchors.center, "center");
            DrawLblA(mTex_fonts[4], "Total  : " + mTotalDimond, 15, 40, "#FFD700", 35, ThreeUI.anchors.right, ThreeUI.anchors.top, "right");

            var keep = "Your Pin code is " + gameuser.PinCode + " keep it,\nyou will be notified if you won";
            DrawLblA(mTex_fonts[5], keep, 2, 72, "#0F0A0A", 45, ThreeUI.anchors.center, ThreeUI.anchors.center, "center");
            DrawLblA(mTex_fonts[6], keep, 0, 70, "#FFFAFA", 45, ThreeUI.anchors.center, ThreeUI.anchors.center, "center");
