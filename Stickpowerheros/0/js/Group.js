...
function DrawGamePlay() {
    ...
    if (isScal == 3) {
        ...
        if (mPlayer.position.x < rod[mRod].plan.scale.y + planBase[mRod].position.x + planBase[mRod].scale.x / 2) {
            ...
            if (mind > line || line > maxd) {
                ...
                isScal = 10;
                mScore += Math.abs(20 - planBase[mNxt].scale.x);
                console.log(line + ", " + sr + ", " + (line > sr - 1) + " " + (line < sr + 1));
    ...
    if (isScal == 12) {
        ...
        if (mPlayer.position.y > -75) {
            ...
            console.log("mPlayer.life = " + mPlayer.life)
            if (mPlayer.life > 0) {
                var temp = mScore;
                GameReset();
                mScore = temp;
...
function GameReset() {
    ...
    mPlayer.rotation.set(0, 0, 0);
    DrawLblA(mTex_fonts[0], "Score : " + mScore, 16, 40, "#fafafa", 35, ThreeUI.anchors.right, ThreeUI.anchors.top, "right");
    console.log("random(0,mp_BG.length) " + random(0, mTex_BG.length));
}

function tap(val) {
    if (GameScreen == GAMEPLAY) {
        console.log(isScal + " isScal " + val);
        if (isScal == 0 && val == 1)
            isScal = val;
        if (isScal == 1 && val == 2)
            isScal = val;
